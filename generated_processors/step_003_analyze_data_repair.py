...
def process(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ek in exp_keys:
        ...
        # 检查数据是否有 NaN 或 Inf
        if np.any(np.isnan(a)) or np.any(np.isnan(v)) or np.any(np.isinf(a)) or np.any(np.isinf(v)):
            raise ValueError(f"实验 {ek} 的数据包含 NaN 或 Inf")
        ...
        # 判断 v 是否几乎常数（方差为零或极小）
        v_var = np.var(v)
        v_is_constant = v_var < 1e-12
        
        if is_constant or is_free:
            # --- 线性回归 ---
            if v_is_constant:
                # v 几乎常数，手动设定：斜率为0，截距为a的均值
                slope_lin = 0.0
                intercept_lin = np.mean(a)
                a_pred_lin = intercept_lin + slope_lin * v
                ss_res_lin = np.sum((a - a_pred_lin)**2)
                ss_tot_lin = np.sum((a - np.mean(a))**2)
                r2_lin = 0.0 if ss_tot_lin < 1e-12 else 1 - ss_res_lin / ss_tot_lin
                rmse_lin = np.sqrt(np.mean((a - a_pred_lin)**2))
                coeff_lin = np.array([slope_lin, intercept_lin])
            else:
            ...
            # 记录线性回归观察（对 constant 和 free 实验都记录）
            lin_reg_type = "linear" if is_constant else "linear_control"
            obs_lin = {
                "summary": f"{ek} {'线性回归' if is_constant else '(free 对照) 线性回归'} a = {coeff_lin[1]:.6f} + {coeff_lin[0]:.6f} * v, R²={r2_lin:.4f}, RMSE={rmse_lin:.6f}",
                "source_data_refs": [f"{ek}:a", f"{ek}:v"],
                "metrics": {
                    "regression_type": lin_reg_type,
                    "experiment_id": ek,
            ...
            if is_constant:
                if v_is_constant:
                    # v 几乎常数，二次项和一次项系数为0，截距为a均值
                    slope2_quad = 0.0
                    slope1_quad = 0.0
                    intercept_quad = np.mean(a)
                    a_pred_quad = intercept_quad + slope1_quad * v + slope2_quad * v**2
                    ss_res_quad = np.sum((a - a_pred_quad)**2)
                    ss_tot_quad = np.sum((a - np.mean(a))**2)
                    r2_quad = 0.0 if ss_tot_quad < 1e-12 else 1 - ss_res_quad / ss_tot_quad
                    rmse_quad = np.sqrt(np.mean((a - a_pred_quad)**2))
                    coeff_quad = np.array([slope2_quad, slope1_quad, intercept_quad])
                else:
