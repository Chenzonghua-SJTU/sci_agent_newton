...
def process(payload: dict) -> dict:
    ...
    # we keep track of per-experiment data for task 3
    per_exp_data = {}
    all_v, all_q, all_res = [], [], []
    for eid in experiment_ids:
        exp = experiments[eid]
    ...
    se_lin = np.sqrt(np.diag(cov_lin))  # intercept, v, q
    ...
    # collect coefficients across experiments
    ...
    # also make a simple residual vs fitted plot for linear model? not required but can be added
    # but keep to requested
    ...
    result = {
        ...
        "derived_series": [],  # no new derived series created
