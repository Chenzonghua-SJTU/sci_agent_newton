...
def process(payload: dict) -> dict:
    ...
    for eid in exp_ids:
        ...
        if field_type == "free" or abs(F_ext) < 1e-12:
            mean_a = np.mean(a_int)
            max_abs_a = np.max(np.abs(a_int))
        ...
        # 初始猜测
        a0_guess = a_int[0] if len(a_int) > 0 else 0.0
        B0_guess = np.max(v_abs) / 2.0 if np.max(v_abs) > 0 else 1.0
        if B0_guess < 0.01:
            B0_guess = 1.0
        try:
    ...
    A_c1 = reg_A.coef_[0]
    A_c2 = reg_A.coef_[1]
    A_intercept = reg_A.intercept_
    A_R2 = reg_A.score(X, y_A)

    B_d1 = reg_B.coef_[0]
    B_d2 = reg_B.coef_[1]
    B_intercept = reg_B.intercept_
    B_R2 = reg_B.score(X, y_B)
    ...
    free_A_mean = df_free['A_fit'].mean() if len(df_free) > 0 else 0.0
    free_A_std = df_free['A_fit'].std() if len(df_free) > 0 else 0.0

    # ---------- 构造证据 ----------
    # 对所有恒外力实验，计算平均R2作为拟合质量指标
    valid_r2 = df_force['R2'].dropna()
    mean_r2 = valid_r2.mean() if len(valid_r2) > 0 else 0.0
    # 自由实验 A 是否接近 0
    ...
    evidence_list.append({
        "hypothesis_id": "H003",
        "supports": mean_r2 > 0.8,  # 如果平均 R2 > 0.8 认为模型数据一致
        "metric_name": "mean_R2_exponential_fits",
        "metric_values": [float(mean_r2)],
        "aggregate_score": float(mean_r2),
    ...
    evidence_list.append({
        "hypothesis_id": "H003",
        "supports": free_a_zero,
        "metric_name": "free_experiment_A_magnitude",
        "metric_values": [float(free_A_mean), float(free_A_std)],
    ...
    evidence_list.append({
        "hypothesis_id": "H003",
        "supports": A_regression_strong,
        "metric_name": "A_multilinear_R2",
        "metric_values": [float(A_R2)],
        "aggregate_score": float(A_R2),
        "experiment_ids": df_force['experiment'].tolist(),
    ...
    evidence_list.append({
        "hypothesis_id": "H003",
        "supports": B_regression_strong,
        "metric_name": "B_multilinear_R2",
        "metric_values": [float(B_R2)],
        "aggregate_score": float(B_R2),
        "experiment_ids": df_force['experiment'].tolist(),
    ...
    # ---------- 构造 metrics ----------
    metrics = {
        "per_experiment_fits": df_force.to_dict(orient='records') if len(df_force) > 0 else [],
        "free_experiment_check": free_exp_a_means,
        "A_multilinear_regression": {
            "c1": float(A_c1),
            "c2": float(A_c2),
            "intercept": float(A_intercept),
            "R2": float(A_R2)
        },
        "B_multilinear_regression": {
            "d1": float(B_d1),
            "d2": float(B_d2),
            "intercept": float(B_intercept),
            "R2": float(B_R2)
        },
        "free_A_mean": float(free_A_mean),
        "free_A_std": float(free_A_std),
    ...
    ax[1].legend()
    fig_path = f"{output_dir}/A_B_vs_F_ext.png"
    plt.tight_layout()
